...
def main():
    ...
    with open(os.path.join(artifact_dir, "profile_export_aiperf.json"), "r") as f:
        result_summary = json.load(f)
        output_token_throughput = result_summary["output_token_throughput"]["avg"]
        time_to_first_token = result_summary["time_to_first_token"]["avg"]

        return {
            "fitness": float(time_to_first_token),
            "signature": [float(time_to_first_token), float(output_token_throughput)],
        }
...
if __name__ == "__main__":
    ...
    if output is not None:
        ...
        print(json.dumps({"output": None, "metainfo": "Error"}))
